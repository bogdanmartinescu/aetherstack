/**
 * Prompt resolution engine for `aether-ui generate`.
 * Maps natural-language UI descriptions to ordered lists of registry items.
 *
 * Shared with packages/mcp-server — keep both in sync if updating recipes.
 */
...
export interface GeneratePlan {
  ...
  components: string[]
  /** Optional starter TSX file content. */
  starterCode?: string
...
export default function DashboardPage() {
  return (
    ...
    notes:
      "dashboard-shell provides the full-page layout. metric-card displays KPIs. Replace placeholder values with real data.",
  },
  {
...
export default function LoginPage() {
  return <LoginBlock />
    ...
    notes: "login-block is a self-contained login screen. Pass onSubmit and onForgotPassword props to wire it to your auth logic.",
  },
  {
...
export default function SignupPage() {
  return <SignupBlock />
    ...
    notes: "signup-block includes name, email, password validation, and terms/privacy links.",
  },
  {
...
export default function SettingsPage() {
  return (
    ...
    notes: "Combine section-header and form-field for structured settings forms.",
  },
  {
...
export default function UsersTable() {
  return (
    ...
    notes: "table-toolbar provides search and filter controls. Place it above a table component.",
  },
]
...
export function resolveGenerate(description: string, availableItems: RegistryItem[]): GeneratePlan {
  const lower = description.toLowerCase()
  const validNames = new Set(availableItems.map((i) => i.name))

  const filter = (names: string[]): string[] => names.filter((n) => validNames.has(n))

  for (const recipe of RECIPES) {
    if (recipe.keywords.some((kw) => lower.includes(kw))) {
      return {
        components: filter(recipe.components),
  ...
  const components = filter([...matched])

  if (components.length === 0) {
    return {
      components: [],
      notes: `Could not match "${description}" to any known Aether UI components. Try a more specific description, or run "aether-ui list" to browse available items.`,
    }
  }

  return {
    components,
    notes: `Matched ${components.length} component${components.length === 1 ? "" : "s"} from description. Install with the command above.`,
  }
}
